...
public class CommandSeek extends Command {
    ...
    public boolean execute(MessageReceivedEvent event, String[] args) {
        ...
        try {
            musicManager.handlers.get(event.getGuild().getIdLong()).trackScheduler.getQueueCopy().get(0).setPosition(Math.min(Integer.parseInt(args[0]) * 1000L, musicManager.handlers.get(event.getGuild().getIdLong()).trackScheduler.getQueueCopy().get(0).getDuration()));
