            ...
            if (event.getChannel().getIdLong() == SHOWCASE) {
                ...
                if (event.getReactionEmote().getAsReactionCode().equalsIgnoreCase(EMOTE)) {
                    ...
                    if (stars == 3 && !posts.getJSONObject(msgID).getBoolean("onBoard")) {
                        event.getGuild()
                                .getTextChannelById(STARBOARD)
                                .sendMessage(":star: **" + stars + "** <#" + SHOWCASE + ">")
                                .queue(message ->
                                        event.getGuild()
                                                .getTextChannelById(SHOWCASE)
                                                .retrieveMessageById(msgID)
                                                .queue(originalMessage -> {
                                                    message.editMessage(new EmbedBuilder()
                                                            .setAuthor(originalMessage.getAuthor().getName(),
                                                                    null,
                                                                    originalMessage.getAuthor().getEffectiveAvatarUrl())
                    ...
                    // Update existing starboard
                        ...
                        event.getGuild().getTextChannelById(STARBOARD).retrieveMessageById(id)
                                .queue(message ->
                                        message
                                                .editMessage(getStarEmoji(finalStars) + " **" + finalStars + "** <#" + SHOWCASE + ">")
                                                .queue());
                    }
